...
TMP           := $(CURDIR)/.tmp
VPATH         := $(TMP)
TFVARS        ?= aws-terraform-salt.tfvars
...
apply_webserver: apply_saltmaster ## Build webserver
	$(call init,$(word 2, $(subst _, ,$@)))
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))

apply_saltmaster: apply_bastion ## Build saltmaster
	$(call init,$(word 2, $(subst _, ,$@)))
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))

apply_bastion: apply_network ## Build bastion
	$(call init,$(word 2, $(subst _, ,$@)))
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))
...
destroy_network: destroy_bastion ## Destroy network
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))

.PHONY: destroy_bastion
destroy_bastion: destroy_saltmaster ## Destroy bastion
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))

.PHONY: destroy_saltmaster
destroy_saltmaster: destroy_webserver ## Destroy saltmaster
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))
...
.PHONY: clean
clean: clean_webserver clean_saltmaster clean_bastion clean_network ## Remove all Terraform working directories
	$(call clean_tmp)
...
.PHONY: clean_saltmaster
clean_saltmaster: ## Remove Terraform working directory of saltmaster
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))

.PHONY: clean_bastion
clean_bastion: ## Remove Terraform working directory of bastion
	$(call $(word 1, $(subst _, ,$@)),$(word 2, $(subst _, ,$@)))
